...
package cn.huwhy.bees.registry.support.command;
...
public class CommandServiceManager implements CommandListener, ServiceListener {
    ...
    static {
        BeesSwitcherUtil.initSwitcher(true);
    }
    ...
    public void notifyService(URL serviceUrl, URL registryUrl, List<URL> urls) {
        ...
        if (registry == null) {
            throw new BeesFrameworkException("registry must be set.");
        }
    ...
    public void notifyCommand(URL serviceUrl, String commandString) {
        ...
        if (!BeesSwitcherUtil.isOpen(MOTAN_COMMAND_SWITCHER) || commandString == null) {
        ...
        for (RpcCommand.ClientCommand command : clientCommandList) {
            ...
            if (match) {
                ...
                if (!CollectionUtil.isEmpty(command.getMergeGroups())) {
                    ...
                    } catch (BeesFrameworkException e) {
                    ...
                    for (String routeRule : command.getRouteRules()) {
                        ...
                        if (matchFrom) {
                            ...
                            while (iterator.hasNext()) {
                                ...
                                if (url.getHost().equalsIgnoreCase("rule")) {
                                    continue;
                                }
                                idx = to.indexOf('*');
                                if (idx != -1) {
    ...
    private List<URL> mergeResult(URL url, Map<String, Integer> weights) {
        ...
        if (weights.size() > 1) {
            ...
            URL ruleUrl = new URL("rule", url.getPort(), url.getPath());
    ...
    private void weightConfigError() {
        throw new BeesFrameworkException("权重比只能是[0,100]间的整数");
    }
