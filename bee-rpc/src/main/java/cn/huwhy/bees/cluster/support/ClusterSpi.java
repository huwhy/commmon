...
import cn.huwhy.bees.rpc.RefererSupports;
...
public class ClusterSpi<T> implements Cluster<T> {
    ...
    public Response call(Request request) {
        ...
        return callFalse(request, new BeesServiceException(BeesErrorMsgConstant.SERVICE_UNFOUND));
    }
    ...
    protected Response callFalse(Request request, Exception cause) {
        ...
        if (Boolean.parseBoolean(getUrl().getParameter(URLParamType.throwException.getName(), URLParamType.throwException.getValue()))) {
            if (cause instanceof BeesAbstractException) {
                throw (BeesAbstractException) cause;
            } else {
                BeesServiceException motanException =
                        new BeesServiceException(String.format("ClusterSpi Call false for request: %s", request), cause);
