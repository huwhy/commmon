...
public class ProviderProtectedMessageRouter extends ProviderMessageRouter {
    ...
    protected Response call(Request request, Provider<?> provider) {
        ...
        String requestKey = BeesFrameworkUtil.getFullMethodString(request);
    ...
    private Response reject(String method, int requestCounter, int totalCounter, int maxThread) {
        ...
        BeesServiceException exception =
                new BeesServiceException("ThreadProtectedRequestRouter reject request: request_counter=" + requestCounter
                        + " total_counter=" + totalCounter + " max_thread=" + maxThread, BeesErrorMsgConstant.SERVICE_REJECT);
