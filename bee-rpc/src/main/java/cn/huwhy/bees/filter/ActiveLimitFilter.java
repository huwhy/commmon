...
import cn.huwhy.bees.core.extension.Activation;
import cn.huwhy.bees.core.extension.SpiMeta;
import cn.huwhy.bees.rpc.RpcStats;
    ...
    public Response filter(Caller<?> caller, Request request) {
        ...
        if (maxAcvitivyCount > 0) {
            ...
            if (activeCount >= maxAcvitivyCount) {
                throw new BeesServiceException(String.format("Request(%s) active count exceed the limit (%s), referer:%s", request,
                        maxAcvitivyCount, caller.getUrl()), BeesErrorMsgConstant.SERVICE_REJECT);
            }
        }
