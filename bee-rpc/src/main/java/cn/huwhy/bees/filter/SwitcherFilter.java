...
public class SwitcherFilter implements Filter {
    ...
    public Response filter(Caller<?> caller, Request request) {
        // 检查接口或方法降级开关状态
        if (BeesSwitcherUtil.isOpen(request.getInterfaceName())
                || BeesSwitcherUtil.isOpen(BeesFrameworkUtil.getFullMethodString(request))) {
    ...
    private Response mockDefaultResponse(Request request) {
        ...
        response.setException(new BeesServiceException("Request false for switcher is on"));
        return response;
    }
