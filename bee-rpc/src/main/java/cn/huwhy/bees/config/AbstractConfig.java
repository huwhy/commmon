    ...
    protected void appendConfigParams(Map<String, String> parameters, String prefix) {
        ...
        for (Method method : methods) {
            try {
                ...
                if (isConfigMethod(method)) {
                    ...
                    if (value == null || StringUtils.isBlank(String.valueOf(value))) {
                        if (configDesc != null && configDesc.required()) {
                            throw new BeesFrameworkException(String.format("%s.%s should not be null or empty", this.getClass()
                                    .getSimpleName(), key), BeesErrorMsgConstant.FRAMEWORK_INIT_ERROR);
                        }
                        continue;
                    }
                ...
                throw new BeesFrameworkException(String.format("Error when append params for config: %s.%s", this.getClass()
                        .getSimpleName(), method.getName()), e, BeesErrorMsgConstant.FRAMEWORK_INIT_ERROR);
            }
        }
    }
